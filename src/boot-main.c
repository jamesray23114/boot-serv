...
void efi_main(EFI_HANDLE* ImageHandle, EFI_SYSTEM_TABLE* SystemTable)
{
    ...
    // load disk driver via efi shell
    EFI_FILE_PROTOCOL* fs = find_root(ImageHandle, ST);

    // create an info structure for multiboot2 and make sure it's aligned to 8 bytes
    ...
    // Add bootloader name to multiboot2 info structure
    ...
    get_gop_info(mb2_info, ImageHandle, ST);
    get_mmap_info(mb2_info, ImageHandle, ST);

    print_info_tags(mb2_info, true, ImageHandle, ST);
    print(u"stalling... ");
    while (true) ;
}
